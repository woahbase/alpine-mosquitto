...
TESTCMD   := \
	uname -a; \
	mosquitto -h | grep -e "\(MQTT\|version\)" \
	#
...
OTHERFLAGS := \
	...
	-e PGID=$(PGID) \
	-e PUID=$(PUID) \
	# -e MOSQUITTO_CONFDIR="/mosquitto/config" \
	# -e MOSQUITTO__log_dest="file /mosquitto/log/mosquitto.log" \
	# -e MOSQUITTO__password_file="/mosquitto/config/.passwd" \
	# -e MOSQUITTO__persistence_location="/mosquitto/data" \
	# -e TZ=Asia/Kolkata \
...
testsub : ## test subscription
	docker exec -it $(CNTNAME) $(CNTSHELL) \
		-c "mosquitto_sub -h localhost -t '#' -u 'mosquitto' -P 'insecurebydefault'"

testpub : ## test publish
	docker exec -it $(CNTNAME) $(CNTSHELL) \
		-c "mosquitto_pub -h localhost -t test -u 'mosquitto' -P 'insecurebydefault' -m 'hello from local'"
	docker exec -it $(CNTNAME) $(CNTSHELL) \
		-c "mosquitto_pub -h localhost -t test -u 'mosquitto' -P 'insecurebydefault' -m 'hello from world' -p 1883"
...
get_config: ## get default config from github
	curl \
		-o $(CURDIR)/root/defaults/mosquitto.conf \
		-SL https://github.com/eclipse/mosquitto/raw/master/mosquitto.conf \
	#
...
$(shell docker run --rm --pull=never \
	...
	-h \
	2>&1 \
	| grep 'mosquitto version ' | sed -e 's_mosquitto version __')
endef
# -- }}}
